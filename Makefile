...
PROJECT_NAME = sleeptransformer
ENV_NAME = sleeptransformer
PYTHON_VERSION = 3.11
...
package:
	...
	conda run -n $(ENV_NAME) $(PYTHON_INTERPRETER) -m pip install -e .

# Create environment and install package
project: create_environment package
...
preprocess-shhs:
	echo "----------------------------------------------------------------------"
	echo "Processing SHHS data"
	python -m ${PROJECT_NAME}.data.preprocess --cohort shhs \
											 --data-dir data/raw/shhs \
											 --output-dir data/processed/shhs \
											 --channels C4 \
											 --fs 100 \
											 --transform stft
